...
const groups = Object.freeze({'All':0, '18+':0, '12-15':0, '16-17':0, '18-24':0, '25-39':0, '40-44':0, '45-54':0, '55-64':0, '65-74':0, '75-84':0, '85+': 0});
...
function getPopulationByAgeGroups(kommuneId) {
    ...
    return fetch('https://data.ssb.no/api/v0/no/table/07459/', {
        ...
        .then(response => {
            const result = {...groups, '0-11':0};
            response.value.forEach((count, age) => {
                if (age < 12) result['0-11'] += count;
                else if (age < 16) result['12-15'] += count;
            ...
            result['18+'] = result['All'] - result['0-11'] - result['12-15'] - result['16-17'];
            return result;
        });
}
    ...
    return Promise.all([jsonp(url + '&fordeling=aar'), jsonp(url + '&fordeling=dag')])
        .then(([total, weekly]) => {
            ...
            for (const dose of doses) {
                ...
                data['18+'][dose].total = data['All'][dose].total - data['12-15'][dose].total - data['16-17'][dose].total;
                for (let i = 0; i <= thisWeekNo; i++) {
                    data['All'][dose].weekly[i] = sum(Object.keys(groups).map(group => data[group][dose].weekly[i]));
                    data['18+'][dose].weekly[i] = data['All'][dose].weekly[i] - data['12-15'][dose].weekly[i] - data['16-17'][dose].weekly[i];
                }
            }
