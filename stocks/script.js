...
function getTimeAggregator(mode) {
  switch(mode) {
    ...
    case 'weekly':
      return date => new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate() + 1 - (date.getDay()||7)));

    case 'monthly':
      return date => new Date(Date.UTC(date.getFullYear(), date.getMonth(), 1));

    case 'quarterly':
      return date => new Date(Date.UTC(date.getFullYear(), 3 * Math.floor(date.getMonth() / 3), 1));

    case 'annually':
      return date => new Date(Date.UTC(date.getFullYear(), 0, 1));
  }
}
...
function createChart(options) {
  ...
  const combined = deep_merge(default_options, options);
